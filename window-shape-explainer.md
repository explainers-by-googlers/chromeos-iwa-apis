...
- **Allowlist**: The IWA origin must be allowlisted to use the `setShape` API.
- **Unframed Mode**: The window must be in the `unframed` display mode.
- **Window management permission**: The origin must be granted the
  `window-management` permission.
- **Minimum shape size**: Rectangles must meet a minimum size requirement of
  10x10 pixels to avoid invisible windows. At least one rectangle in the
  custom shape must meet this requirement.
